...
After using my ILS implementation to generate routes in and around Schenectady NY, I identified the following limitations of the algorithm.

\begin{itemize}
    \item The algorithm uses the DFS heuristic (Algorithm \ref{alg:dfs}) to build the initial solution. Since the search depth is limited by the $maxDepth$ parameter, the DFS may not find an initial solution between locations that are far apart.
    \item The DFS heuristic is very slow in road-dense areas even with a small $maxDepth$ parameter. There are clearly too many search paths to check. It makes sense that \citeauthor{lu2015arc} are using spatial techniques instead of DFS to solve this problem.
    \item There is no turn costing built into the algorithm. Routes with a high amount of turns can be dangerous since the cyclist has to constantly cross traffic. Figure \ref{fig:graphhopper-turnissue} shows two quick successive turns instead of staying straight on the road.
    \item The algorithm relies on precomputed all-pairs shortest path for doing arc feasibility checking. For large road graphs, this can be very impractical. This isn't really a huge limitation because most routing engines rely on some kind of preprocessing step such as contraction hierarchies.
\end{itemize}


\begin{figure}[h]
    \begin{center}
        \includegraphics[width=\textwidth]{figs/graphhopper_route}
    \end{center}
    \caption{GraphHopper web frontend with route generated by ILS (Algorithm \ref{alg:ils-ver})}
    \label{fig:graphhopper-route1}
\end{figure}

\begin{figure}[h]
    \begin{center}
        \includegraphics[width=\textwidth]{figs/route_turn}
    \end{center}
    \caption{Dangerous turn produced by Algorithm \ref{alg:ils-ver} due to lack of turn costing. This path feature is inset in the middle of Figure \ref{fig:graphhopper-route1}.}
    \label{fig:graphhopper-turnissue}
\end{figure}


\section{Next Term Plan}
I have yet to implement the Lu-Shahabi algorithm using the GraphHopper engine. This is the first thing that I plan to do in my research for next term. \citeauthor{lu2015arc} claim that their algorithms are much faster than the original VVA ILS algorithm. I want to see if this really is the case on my road network of upstate New York. Once both algorithms are implemented, I would like to do some more in-depth performance analysis of the two.

In addition, I would like to implement a few modifications to the existing ILS algorithms and analyze their effectiveness at producing better routes or faster solutions:

\begin{itemize}
    \item It is clear from Figure \ref{fig:graphhopper-turnissue} that the VVA algorithm can lead to dangerous routes because it is not taking into account turns. I want to see if this can be improved by penalizing turns in the route.
    \item I would like to see if the route can be improved by not using the DFS heuristic to build the initial solution. Could a shortest path algorithm be used instead?
    \item Currently roads in the VVA algorithm are scored using the ``road priority" determined by GraphHopper's bike routing profile. This is an arbitrary number which determines the preferability of the road using OSM metadata specific to bikes. I would like to see if this can be improved.
    \item Currently, both of the ILS algorithms discussed do not factor in the shape of the route. While both algorithms can definitely generate routes that start and end in the same place (i.e. circular), they may not produce a single ``loop" as evidenced by Figure \ref{fig:graphhopper-route1}. It would be interesting to factor in the shape/area of the route into these algorithms to score roads with a convex shape higher. 
    \item Perhaps design and implement a new algorithm with GraphHopper. Maybe an existing directed search algorithm like A* could be modified to produce a convex shaped route.
\end{itemize}


\subsection{Tentative Term Schedule}
The following list is by term week number.
\begin{enumerate}
    \item (Jan 3 - Jan 9) Finish up work on GraphHopper implementation of Lu-Shahabi algorithm. Continue brainstorming algorithm improvements.
    \item (Jan 10 - Jan 16) Implement turn costing in VVA algorithm and performance analysis between VVA and Lu-Shahabi.
    \item (Jan 17 - Jan 23) Begin work on algorithm which factors in route shape.
    \item (Jan 24 - Jan 30) Continued algorithm work.
    \item (Jan 31 - Feb 6) Continued algorithm work.
    \item (Feb 7 - Feb 13) Continued algorithm work.
    \item (Feb 14 - Feb 20) Continued algorithm work.
    \item (Feb 21 - Feb 27) Begin work on poster and thesis writeup.
    \item (Feb 28 - Mar 6) Finalize poster and continue work on thesis.
    \item (Mar 7 - Mar 13) Finalize thesis.
\end{enumerate}
